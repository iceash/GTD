...
class SinaSDK extends ThinkOauth{
	...
	protected $ApiUpload = "https://api.weibo.com/2/";

	protected $Authorize='with_offical_account=1&scope=follow_app_official_microblog';
	 ...
	 * @return json
	 */
	public function call($api, $param = '', $method = 'GET', $multi = false,$isupload = false){		
		/* 新浪微博调用公共参数 */
		$params = array(
		...
		if($isupload===true){
			$url=$this->ApiUpload.$api.'.json';
		}
		else{
			$url=$this->url($api, '.json');
		}
		//var_dump($vars);
		//var_dump($this->url($api, '.json'));
		//
		//var_dump($url);
		//exit;
		$data = $this->http($url, $vars, $method, array(), $multi);
